...
const Header: React.FC = () => {
  ...
  const [keepYearVisible, setKeepYearVisible] = useState(false);
  ...
  useEffect(() => {
    ...
    // Mark as started so even if we navigate away early, 
    // we don't restart the whole sequence next time.
    // However, we mark it fully complete later for the UI transition.
    ...
    // Show Lagentry logo with 2025
    timers.push(window.setTimeout(() => {
      setShowCopyright(false);
      setShowLagentry(true);
      setKeepYearVisible(true); // Keep 2025 visible after intro
    }, currentDelay));
    currentDelay += 2000;

    // End intro - keep 2025 visible during transition
    timers.push(window.setTimeout(() => {
      setShowNavigation(true);
      // Keep showLagentry true for smooth transition
      ...
      setTimeout(() => {
        ...
        setIsActive(false);
        // Keep 2025 visible even after header hides
      }, 500);
    }, currentDelay + 1000));

    return () => {
      timers.forEach((id) => window.clearTimeout(id));
      // Ensure it's marked as completed if the user navigates away during animation
      if (isActive) {
        markIntroCompleted();
      }
    };
  ...
  return (
    <header className={`header ${hideHeader ? 'header-hidden' : ''} ${showLagentry ? 'lagentry-fullscreen' : ''}`}>
      ...
      >
            ...
            {currentWordIndex >= 0 && (
              <>
                {currentWordIndex === 2 ? (
                  ...
                  <h2
                    key={`phrase-${currentWordIndex}`}
                    className={`word-animation phrase-${currentWordIndex} ${currentWordIndex === 0 || currentWordIndex === 1 ? 'typewriter-reveal' :
                        'slide-fade-reveal'
                      }`}
                  >
        ...
        {/* Lagentry Year + Logo */}
        <div className={`background-lagentry-text ${showLagentry || keepYearVisible ? 'visible' : ''}`}>
          <span className="lagentry-year">2025</span>
          {showLagentry && <img src={lagentryImage} alt="LAGENTRY logo" className="lagentry-image" />}
        </div>
      </div>
    </header>
  );
